    ...
    <script>
        ...
        document.getElementById('btn-pay-now-midtrans').addEventListener('click', async function() {
            try {
                ...
                if (data.snap_token) {
                    window.snap.pay(data.snap_token, {
                        onSuccess: async function(result){
                            // Hit backend to update status immediately
                            try {
                                await fetch(callbackRoute, {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        ...
                        onClose: function(){
                            // User closed the popup, do nothing or show a small toast
                        }
                    });
                } else if (data.error) {
                ...
                console.error(error);
                this.innerHTML = `<svg class="w-5 h-5 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path></svg> Bayar Sekarang`;
                this.disabled = false;
            }
        });
    </script>
