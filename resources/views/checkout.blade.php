<x-layout>
            ...
            <div class="lg:col-span-5 space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                    ...
                    <div id="form-midtrans" class="hidden text-center py-6">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 px-4 mb-6">
                            Lanjutkan untuk melakukan pembayaran menggunakan QRIS, Virtual Account, atau metode lain yang tersedia.
                        </p>
    ...
    <script>
        ...
        const midtransRoute = '{{ route('payment.midtrans', $order->order_number) }}';
        
        const callbackRoute = '{{ route('payment.midtrans.callback', $order->order_number) }}';
        ...
        let midtransPollInterval = null;

        function stopMidtransPolling() {
            if (midtransPollInterval) {
                clearInterval(midtransPollInterval);
                midtransPollInterval = null;
            }
        }

        async function verifyPaymentStatus() {
            try {
                const res = await fetch(callbackRoute, {
                    method: 'POST',
                    headers: {
                ...
                console.error('Verifikasi status error:', err);
            }
            return false;
        }

        document.getElementById('btn-pay-now-midtrans').addEventListener('click', async function() {
            const btn = this;
            ...
            try {
                ...
                const response = await fetch(midtransRoute, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                ...
                if (data.snap_token) {
                    ...
                    // Auto polling setiap 3 detik agar mendeteksi saat pengguna selesai bayar di smartphone
                    midtransPollInterval = setInterval(verifyPaymentStatus, 3000);

                    // Batas maksimal polling 10 menit
                    setTimeout(stopMidtransPolling, 600000);
                    ...
                    window.snap.pay(data.snap_token, {
                        ...
                        onClose: async function(){
                            // Jika pengguna menutup modal (misal setelah melihat checkmark hijau), cek status satu kali lagi
                    ...
                    alert('Gagal mengambil token: ' + data.error);
                    btn.innerHTML = originalText;
                    btn.disabled = false;
                }
            } catch (error) {
                stopMidtransPolling();
                alert('Terjadi kesalahan pada sistem: ' + (error.message || 'Silakan coba lagi.'));
                console.error(error);
                btn.innerHTML = originalText;
                btn.disabled = false;
            }
